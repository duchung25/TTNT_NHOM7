...
class NaiveBayes:
    def fit(self, X, y):
        ...
        self.means = {} #Trung bình của các đặc trưng
        self.vars = {} #Phương sai của các đặc trưng cho mỗi lớp
        self.priors = {} #Xác suất tiên nghiệm cho mỗi lớp
    ...
    def predict_proba(self, X):
        probs = []
        for x in X:
            class_probs = [] #Khởi tạo một danh sách xác suất lớp
            for c in self.classes:
                prior = np.log(self.priors[c]) #Xác suất tiên nghiệm
                conditional = np.sum(np.log(self.gaussian_prob(x, self.means[c], self.vars[c]))) #xác suất có điều kiện
